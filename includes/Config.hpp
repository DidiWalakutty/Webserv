...
struct Redirect
{
	int statusCode = 0;          // 0 = no redirection
	std::string targetURL;
};

// Configuration related to a single URL path
// Each LocationParse corresponds to a single "route" in the server
struct LocationParse
{
	...
	std::string index;  		// Default file to serve if URL is a directory */
	...
	std::vector<std::string> cgi_executable;
	
	size_t maxBodySize = 0;
	Redirect redirect;			// Optional redirect
};
